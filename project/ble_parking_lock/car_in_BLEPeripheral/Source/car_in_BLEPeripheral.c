...
#define  CONNECTED_OBJ_IS_INCORRECT_TIMEOUT   4000 //4sÖÓÖ®ºó¼ì²éÁ¬½Ó¶ÔÏóÊÇ·ñÕýÈ·
...
static void adv_inderct_addr_update()
{
  if(addr_update_flag)//Èç¹ûÒÑ¾­ÊÇÁ¬½Ó¹ýµÄÇé¿ö
  {
  addr_update_flag=FALSE;
  GAP_SetParamValue(TGAP_GEN_DISC_ADV_MIN,12000);//ÉèÖÃgeneral¹ã²¥³¬Ê±ÊÂ¼þ 0 =ÓÀÔ¶£¬ÊÂ¼þwaiting 12s
  }
  else
  {
  GAP_SetParamValue(TGAP_GEN_DISC_ADV_MIN,1500);//ÉèÖÃgeneral¹ã²¥³¬Ê±ÊÂ¼þ 0 =ÓÀÔ¶£¬ÊÂ¼þwaiting 1.5s 
  }
...
uint16 SimpleBLEPeripheral_ProcessEvent( uint8 task_id, uint16 events )
{
    ...
    if ( events & CONNECTED_OBJ_IS_INCORRECT_EVT )
  {
   ...
   if( min_con!=max_con || max_con!=DEFAULT_DESIRED_MAX_CONN_INTERVAL || to!=DEFAULT_DESIRED_CONN_TIMEOUT ||lat!=DEFAULT_DESIRED_SLAVE_LATENCY)//Á¬½Ó¶ÔÏó´íÎó
   { 
