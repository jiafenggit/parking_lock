...
static uint8 controlBLECentralEventCB( gapCentralRoleEvent_t *pEvent );
...
uint16 SimpleBLECentral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    if(simpleBLEState == BLE_STATE_CONNECTING)//»Áπ˚“¿»ª‘⁄µ»¥˝¡¨Ω” ÷§√˜¡¨Ω”π˝≥Ã≥¨ ±
    {
    app_write_string("\r\n ‘ÕºΩ®¡¢¡¨Ω”π˝≥Ã≥¨ ±!πÿ±’est«Î«Û!");
...
static void wkxboot_write_car_in_info_in_peer()
{
  ...
  if ( simpleBLEState == BLE_STATE_CONNECTED &&
              ...
              simpleBLEProcedureInProgress == FALSE )
    {
       ...
       *((ble_device_t*)req.pValue) = car_in_info;
       req.sig = 0;
       req.cmd = 0;
 ...
 if ( keys & KEY_FUN1 )
  {
              ...
              simpleBLEProcedureInProgress == FALSE )
    {
      ...
      if ( simpleBLEDoWrite )
      {
        ...
        req.handle = BLE_lock_in_CharHdl;
        req.len = sizeof(ble_device_t);;
        *((ble_device_t*)req.pValue) = lock_in_info;
        req.sig = 0;
        req.cmd = 0;
        status = GATT_WriteCharValue( simpleBLEConnHandle, &req, controlBLETaskId );    
        app_write_string("\r\nø™ º–¥÷µ!status:");
...
static void controlBLECentralProcessGATTMsg( gattMsgEvent_t *pMsg )
{
         ...
         ( pMsg->msg.errorRsp.reqOpcode == ATT_READ_REQ ) ) )
  {
    ...
    else 
    {
      ...
      {
      lock_in_info = *((ble_device_t*)pMsg->msg.readRsp.pValue);
      app_write_string("\r\n∂¡µΩlock inµƒ÷µ «:\r\n");
      ...
      {
      car_in_info = *((ble_device_t*)pMsg->msg.readRsp.pValue);
      app_write_string("\r\n∂¡µΩcar inµƒ÷µ «:\r\n");
      ...
      {
      time_stamp_info = *((time_stamp_t*)pMsg->msg.readRsp.pValue);
      app_write_string("\r\n∂¡µΩtime stampµƒ÷µ «:\r\n");
...
static void wkxboot_connect()
{
    ...
    if ( simpleBLEState == BLE_STATE_IDLE )
    {
       ...
       if(SUCCESS==rt_status)
       {
         ...
         osal_start_timerEx(controlBLETaskId,ATTEMPT_TO_EST_CONN_TIMEOUT_EVT,ATTEMPT_TO_EST_CONN_TIMEOUT_VALUE);//Ω®¡¢¡¨Ω”µƒπ˝≥Ã≥¨ ±
         app_write_string("\r\nø™ ºº‡ ”¡¨Ω”≥¨ ±...");
       }
       else
       {
        ...
        // LL_CreateConnCancel();
         ...
         app_write_string("\r\n…‘∫Û÷ÿ–¬ø™ º…®√Ë!");
         osal_start_timerEx(controlBLETaskId,START_SCAN_EVT,DEFAULT_START_TO_SCAN_DELAY);
       }
    }      
    else
    {
...
static uint8 controlBLECentralEventCB( gapCentralRoleEvent_t *pEvent )
{
  switch ( pEvent->gap.opcode )
  {
    ...
    case GAP_DEVICE_INFO_EVENT:
      {
        ...
        {
          osal_memcpy(ble_tar_addr,pEvent->deviceInfo.addr,B_ADDR_LEN);//øΩ±¥ƒø±Íµÿ÷∑
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:
      ...
      app_write_string("\r\nÕÍ≥…“ª∏ˆ…®√Ë÷‹∆⁄ ¬º˛!");  
      if(simpleBLEState == BLE_STATE_IDLE)//»Áπ˚ «ø’œ–◊¥Ã¨æÕø™∆Ù…®√Ë
       {
       osal_start_timerEx(controlBLETaskId,START_SCAN_EVT,DEFAULT_START_TO_SCAN_DELAY);      
       app_write_string("\r\nø’œ–◊¥Ã¨Ω´ºÃ–¯…®√Ë!");
       }
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        osal_stop_timerEx(controlBLETaskId,ATTEMPT_TO_EST_CONN_TIMEOUT_EVT);//πÿ±’Ω®¡¢¡¨Ω”µƒπ˝≥Ã≥¨ ±∂® ±∆˜
  ...
  if ( simpleBLESvcStartHdl != 0 )
      {
       ...
       uint8 status;
       status= GATT_ReadUsingCharUUID( simpleBLEConnHandle, &req, controlBLETaskId );
       app_write_string("\r\nstatus:");
       app_write_string(uint8_to_string(status));
      }
}
...
static void ble_control_handle_send_queue()
{
  ...
  {
   app_write_string("\r\nÕ¯¬ÁŒ¥≥ı ºªØŒ™UDP,Œﬁ∑®”Î∑˛ŒÒ∆˜Õ®–≈!÷‹∆⁄¬÷—Ø∑¢ÀÕ∂”¡–over!");
   return ;
  }
