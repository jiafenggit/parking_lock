...
#define DEFAULT_UPDATE_CONN_TIMEOUT           250 //¶Ï¿ªÁ¬½Ó³¬Ê±Ê±¼äÉèÖÃ
...
#define  CAR_EXSIT_TRIGGER_VALUE              2 //2´ÎÃ»ÓÐ¼ì²âµ½³µÁ¾¾ÍÁ¢Æðµµ¸Ë
...
void lock_in_BLECentral_Init( uint8 task_id )
{
  ...
  register_for_scan_car(lock_in_BLETaskId);
...
uint16 lock_in_BLECentral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    if(simpleBLEState != BLE_STATE_CONNECTED)//Ã»ÓÐÁ¬½ÓµÄÇé¿öÏÂÉ¨Ãè³µÁ¾´æÔÚÐÅÏ¢
...
static void lock_in_BLECentral_Handlebatt(uint8 batt)
{
  owner_info.ble_battery=(uint8)(batt*(BAT_VOLTAGE_SCALE+1)/BAT_FULL_VOLTAGE);//Ô¼µÈÓÚn*100*1.25*(BAT_VOLTAGE_SCALE+1)/BAT_FULL_VOLTAGE*128; 
  app_write_string("\r\n¸üÐÂowner info batt Íê³É,µ±Ç°owner info batt:");
...
static void wkxboot_process_park_authority()
{
  ...
  {   
   app_write_string("\r\nÄ¿±êÀ¶ÑÀÓÐÊÚÈ¨,¿ÉÒÔÍ£³µ!");
   app_motor_set_target_state_bottom();//·ÅÏÂµµ¸Ë  
  }
  else
  {
  ...
  if(pMsg->state==MOTOR_ON_BOTTOM_STATE)
   {
    ...
    //app_write_string("\r\n¿ªÆô³£×´Ì¬³µÁ¾É¨Ãè!"); 
    //if(simpleBLEState == BLE_STATE_CONNECTED )//ÔÚ¸Õ½¨Á¢Á¬½ÓµÄÊ±ºò
  ...
  switch ( pEvent->gap.opcode )
  {
    case GAP_DEVICE_INIT_DONE_EVENT:  
      ...
      /***wkxboot***/
        ...
        app_motor_set_target_state_top();//init stateµµ¸ËÊúÆð
      }
      break;
    ...
    case GAP_DEVICE_INFO_EVENT:
      {
        ...
        {
          app_motor_set_target_state_bottom();//½µÏÂµµ¸Ë
          osal_stop_timerEx(lock_in_BLETaskId,START_SCAN_CAR_EXSIT_EVT);
          app_write_string("\r\n¹Ø±ÕÆ½³£³µÁ¾É¨Ãè!");
          
         // car_exsit_cnt=0;//²»´æÔÚ´ÎÊý=0
    ...
    case GAP_DEVICE_DISCOVERY_EVENT:
      {
        // discovery complete
        simpleBLEScanning = FALSE;
       // osal_start_timerEx(lock_in_BLETaskId,START_SCAN_CAR_EXSIT_EVT,START_SCAN_CAR_ON_NORMAL);//¼ì²â³µÁ¾ÐÅÏ¢
       /****wkxboot***/
        /*
        ...
        else if (simpleBLEScanRes <= DEFAULT_MAX_SCAN_RES)
        {
          ...
          app_write_string("\r\nÉ¨Ãèrssi:");
          app_write_string(uint8_to_string(pEvent->deviceInfo.rssi));
...
static void simpleBLEGATTDiscoveryEvent( gattMsgEvent_t *pMsg )
{
  ...
  else if ( simpleBLEDiscState == BLE_DISC_STATE_LOCK_IN_CHAR )
  {
    ...
    {  
      ...
      lock_in_info = *((ble_device_t*)&pMsg->msg.readByTypeRsp.dataList[2]);//±£´ælock_in_infoÐÅÏ¢
  ...
  else if ( simpleBLEDiscState == BLE_DISC_STATE_CAR_IN_CHAR )//·¢ÏÖÁË³µÔØÀ¶ÑÀÐÅÏ¢¾ä±úÐÅÏ¢£¬´ËÐÅÏ¢°üÀ¨¾ä±úºÍ´æ´¢µÄÊý¾Ý
  {
    ...
    if ( pMsg->method == ATT_READ_BY_TYPE_RSP)
    {
      if(  pMsg->msg.readByTypeRsp.numPairs > 0 )
     {
      ...
      car_in_info = *((ble_device_t*)&pMsg->msg.readByTypeRsp.dataList[2]);//±£´æcar_in_infoÐÅÏ¢
