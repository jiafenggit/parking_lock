...
uint16 lock_in_BLECentral_ProcessEvent( uint8 task_id, uint16 events )
{
  ...
  {
    if(simpleBLEState == BLE_STATE_IDLE)//ÔÚ¿ÕÏÐÇé¿öÏÂÉ¨Ãè³µÁ¾´æÔÚÐÅÏ¢
    {
    app_start_to_scan_car();
    }
    else
    {
     app_write_string("\r\nµ±Ç°·Ç¿ÕÏÐ×´Ì¬,ÖÕÖ¹³µÁ¾É¨Ãè!");
    }
    ...
    case GAP_LINK_ESTABLISHED_EVENT:
      {
        ...
        else
        {
          ...
          osal_start_timerEx(lock_in_BLETaskId,START_SCAN_CAR_EXSIT_EVT,START_SCAN_CAR_ON_NORMAL);//µ«ÊÇÒªÒ»¶ÎÊ±¼äºóÉ¨Ãè³µÁ¾
        }
      }
      break;
