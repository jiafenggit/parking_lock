...
extern uint8 OnBoard_ScanCarCallback (uint8 scan_time);
/*
 * Perform a soft reset - jump to 0x0
 */
extern __near_func void Onboard_soft_reset( void );

extern uint8 register_for_scan_car(uint8 task_id);
extern uint8 register_for_motor(uint8 task_id);
extern uint8 register_for_batt(uint8 task_id);
