...
#if HAL_SCAN_CAR > 0
  if(events & HAL_SCAN_CAR_LATENCY_EVENT)
  {
    hal_process_scan_car_latency_event();
    return events ^ HAL_SCAN_CAR_LATENCY_EVENT;
  }
#endif
 
#if HAL_MOTOR_CHECK > 0

  if(events & HAL_MOTOR_CHECK_SPEED_EVENT)
  {
    hal_process_motor_check_speed_event();
    return events ^ HAL_MOTOR_CHECK_SPEED_EVENT;
  }
  
  if(events & HAL_MOTOR_CHECK_BLOCK_EVENT)
  {
    hal_process_motor_check_motor_block_event();
    return events ^ HAL_MOTOR_CHECK_BLOCK_EVENT;
  }
  if(events & HAL_MOTOR_VERIFY_EVENT)
  {
    hal_process_motor_verify_event();
    return events ^ HAL_MOTOR_VERIFY_EVENT;
    
  }
#endif
#if HAL_BATT_CHECK > 0
   if(events & HAL_BATT_CHECK_EVENT)
  {
    hal_process_update_batt_info_event();
    return events ^ HAL_BATT_CHECK_EVENT;
  }
#endif 
